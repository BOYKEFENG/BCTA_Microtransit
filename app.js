require([
...
], function(Map, MapView, FeatureLayer, LayerList, Legend, Expand) {
    ...
    const combinedBG = new FeatureLayer({
      url: "https://services1.arcgis.com/HmwnYiJTBZ4UkySc/arcgis/rest/services/Aliquippa_BG_with_OD/FeatureServer/3",
      id: "Combined_BG",
    ...
    const odTable = new FeatureLayer({
      url: "https://services1.arcgis.com/HmwnYiJTBZ4UkySc/arcgis/rest/services/Aliquippa_BG_with_OD/FeatureServer/75",
      id: "OD_Table",
      outFields: ["*"],  // This will fetch all fields
      visible: true,
      opacity: 0.7,
    ...
    map.addMany([combinedBG, odTable, destBG]);

    // 3. LayerList widget with expansion logic
    let selectedBGId = null;
    const layerList = new LayerList({
      view: view,
      listItemCreatedFunction: function(event) {
        const item = event.item;
        // Expand the item whose ID matches the clicked BG_ID
        if (item.layer.id === selectedBGId) {
          item.open = true;
        }
      }
    });
    ...
    toggleButton.className = "esri-button";
    toggleButton.innerHTML = "Toggle Layer Visibility";
    toggleButton.addEventListener("click", function() {
        odTable.visible = !odTable.visible;
        combinedBG.visible = !combinedBG.visible;
    });
